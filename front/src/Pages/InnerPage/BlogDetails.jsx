...
const BlogDetails = () => {
  ...
  const { id } = useParams();
  const [articles, setDataArticles] = useState("");
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setCurrent(articles.findIndex((art) => art.id == article.id));
            ...
            <div className="pt-5 lg:pt-[35px]  pr-3">
              <div data-aos="fade-up" data-aos-duration="1000">
                <p className="text-base font-Garamond text-gray dark:text-lightGray">
                  ...
                  {category &&
                    category
                      .filter((cat) => cat.id === article.category)
                      .map((items, index) => (
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  {tag &&
                    tag
                      .filter((t) => article.tag.includes(t.id))
                      .map((items, index) => (
                        <span
                          key={index}
                          className="text-sm border-[1px] border-lightGray dark:border-gray px-3 py-1 dark:text-white"
                        >
                          {items.name}
                        </span>
              ...
              <div className="lg:flex items-center gap-5  ">
                ...
                >
                  <Link
                    to={`/blog_details/${articles[prev]?.id}`}
                    className="flex items-center"
                  >
                    <img
                      src={`http://127.0.0.1:8000${articles[prev]?.image}`}
                      className=" mr-3 2xl:mr-5 h-20 w-20"
                      alt=""
                    />
                ...
                >
                  <Link
                    to={`/blog_details/${articles[next]?.id}`}
                    className="flex flex-row-reverse items-center"
                  >
                    <img
                      src={`http://127.0.0.1:8000${articles[next]?.image}`}
                      className=" ml-3 2xl:ml-5 h-20 w-20 "
                      alt=""
                    />
                    <div className="text-left">
                      <h4 className="text-base 2xl:text-lg leading-6 text-[#101010] dark:text-white font-medium font-Garamond hover:underline underline-offset-4">
